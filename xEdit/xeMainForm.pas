      ...
      try
        ...
        try
          ...
          try
            ...
            reMain.SelText := CRLF + CRLF + 'BB-84C/TES5Edit Automation 4.1.6 r1..r9' + CRLF + CRLF;
            ...
            reMain.SelText :=
              ...
              'r9 (this release):' + CRLF +
              '  - Pending-save readback (issue #6): session.get_dirty_state and the dirtyState sub-object of session.save now report pendingShutdownFiles (with the queued temp filename) and pendingShutdownCount. Saving a memory-mapped loaded plugin writes full bytes to a .save.<ts> temp file and defers the final rename to shutdown; dirty:false can now coexist with a visible pending queue instead of being a false durability all-clear. Capability supports.pendingSaveReadback.' + CRLF +
              '  - session.flush (issue #6): in-band drain of the pending-rename queue with per-file results (flushedFiles: fileName/renamed/error), a pendingRemaining readback, and a clean daemon self-exit after the response is written and flushed. Refuses dirty sessions (state_conflict) unless force:true; failed renames stay queued for the process-exit retry. Capability supports.sessionFlush.' + CRLF +
              '  - Strict script FormID resolver (issue #7): RecordByFormIDStrict resolves a load-order-space FormID through its owning file and returns nil unless that owner is the target file itself or one of its masters, eliminating wrong-but-valid record returns from foreign FormIDs. Legacy RecordByFormID behavior is unchanged.' + CRLF +
              '  - Script integer formatters (issue #7): IntToStr64 and the two-argument IntToHex are registered and admitted by the script policy ledger (64-bit safe; plain IntToHex no longer aborts mid-run for FormID-sized values).' + CRLF +
              '  - Sortable-container notice (issue #7): elements.set_value / set_native_value report sortInvalidated plus an advisory notice when a successful write lands in a sorted container (LVLI-style arrays re-sort lazily, so index-based locators may move after the write).' + CRLF +
              '  - Script policy preflight (issue #7): entry scripts are scanned after compile and BEFORE Initialize; a call to a symbol outside the policy ledger, host-global, declared-routine, and keyword/type allowlists now fails non-mutating with policyPreflight + preflightLine/Column, instead of aborting mid-run after mutations. The pure JVCL stock builtins (Length, Format, Trim, Copy, UpperCase, IntToStr, ...) are now ledger-admitted so ordinary scripts keep working.' + CRLF +
              '  - Partial-mutation reporting (issue #7): failed scripts report mutationsAppliedBeforeFailure, modifiedFilesBeforeFailure, and preExistingDirtyFiles so lifecycle clients know whether a failed run left changes behind.' + CRLF +
              '  - Capability contract advanced from 0.22 to 0.23 with supports.pendingSaveReadback, supports.sessionFlush, and supports.sortableContainerNotice.' + CRLF +
              '  - Internal version bumped from 4.1.6r8 to 4.1.6r9; update check follows v4.1.6-automation.9 tag.' + CRLF + CRLF +
              'r8:' + CRLF +
