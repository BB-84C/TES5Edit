...
The current local truth on this branch is now the accepted Phase 15D `records.create` parent-spec surface on top of the accepted Phase 15A/B/C ChildGroup progressive-disclosure, apply-filter, and collector foundations, Phase 14 r5 UTF-8 inline-decode default, Phase 13 `elements.*` mutation surface expansion, Phase 12 Starfield plain `.esp` write enablement, Phase 11 navigate-to-record, and the scoped xEdit-side `1.0` freeze:
