...
FO4 harness migration (2026-08-11): the deleted `D:` portable harness has been replaced by `B:\WastelandBlues 2.0`. The canonical runtime is `B:\WastelandBlues 2.0\Stock Game\Fallout 4\Tools\FO4Edit\FO4Edit.exe`, launched through `D:\awesome-bgs-mod-master\tools\mo2-vfs-launcher\xedit-client.ps1 process launch` and the in-GUI Mo2AgentControl broker. Historical phase narratives below remain phase records; their operational target/entrypoint references now defer to this migration note and `AGENTS.md`.
...
- Phase 17B REVERSAL + full registry capture (2026-07-08 continuation) SUPERSEDES the "DumpReflection dead end" verdict immediately above. HISTORICAL/SUPERSEDED: the v2/v3 "registry not walkable / hash-keyed" conclusion was a methodology flaw, not a fact. The registry IS walkable; a full authoritative dump was captured from live Starfield 1.16.244.0. Evidence: `.opencode/artifacts/issue5-pndt-reflection-copy/refl-probe/v4-run1.log` / `v5-run1.log` / `v6-run1.log`; dump `refl-probe/reflection-registry.json` (314 KB, 669 classes); reversal writeup `multi-lens/registry-capture-2026-07-08-reversal.md`. Probe source `tools/ReflProbe/` (read-only OpenProcess + VirtualQueryEx + ReadProcessMemory; no injection, no writes).
  - Root cause of the earlier wrong verdict: v2/v3 searched for inbound pointers to the FIRST heap copy of a class-name string, which was a REFL-payload copy (serialized ESM buffer), not the registry's interned .rdata copy. Pointers to the registry copy live at a different address, so v2/v3 could not have found them.
  - Corrected method: (1) track ALL 334 copies of one class-name anchor + single-pass inbound-pointer index -> exactly ONE copy (module+0x4B567C0, .rdata) is pointed at, by a .data ClassType node; (2) that ClassType node matches the 1.8.86 layout field-for-field (vftable@0, {Size/Align/Kind/Flags}@8, Next@0x10, Name@0x18, Ctor/Dtor/Move/Copy@0x20-0x38, Properties StdVector@0x40); (3) ROOT CAUSE of the original DumpReflection failure was NOT layout drift but v5 Address-Library ID renumbering handing it a wrong list-head (id 885835 -> unrelated ActorValue data in 1.16.244); anchor-seeding bypasses Address Library entirely; (4) the ClassType vftable (module+0x4D9A808) is shared by every node, so one memory pass enumerates ALL 669 ClassType nodes, including the gap classes librarians could not resolve (BSAttachConfig::LightAttachment/ObjectAttachment/ParticleAttachment/SoundAttachment/LensFlareAttachment, BGSAudio::WwiseSoundHook); (5) ClassProperty layout recovered (stride 0x38: Name@0, Type@8, Offset@0x10, attr-region@0x18-0x28, sentinel@0x30).
  - THE DISCRIMINATOR: `field.typeKind == 9` == form-reference pointer (BorrowedPointer-to-TESForm), registry-authoritative and version-matched. Every kind=9 field carries the "p" name prefix and cross-matches a CommonLibSF FormReference field exactly (BGSWeatherSettingsForm's 10 kind=9 fields == CommonLibSF's 10 documented form pointers; BSAttachConfig::LightAttachment.pLightForm==LIGH resolved from registry alone; BGSAtmosphere::OverrideSettings.pSunPresetOverride==SUNP). typeKind 3=struct, 0=scalar, 1=string, 4=array (element kind must be resolved). BSHoudini::HoudiniAssetData has ZERO kind=9 fields -> registry-authoritative confirmation the PNDT/JaffaII Houdini payload has NO game FormIDs and is safe for verbatim copy.
  - Inherited by later phases: (a) the 17C-2 whitelist substrate is now AUTHORITATIVE and version-matched (not partial header-inference); `reflection-registry.json` covers all 669 classes. (b) FormID-slot identification for 17C-2/17C-3 = `typeKind == 9` (plus kind=4 arrays whose element kind is 9), derivable OFFLINE from the saved JSON — NO live process needed for issue #5 or remap wiring. (c) The only live-process-only refinement left is resolving each kind=9 field to its target FormType (KYWD/IMGS/LIGH/...), which is UI-validation scope and DEFERRED; remap only needs "this slot is a FormID." (d) Gibbed DumpReflection, IDA, x64dbg, CKPE/CK2 are NOT needed for the whitelist — the anchor-seeded ReflProbe walk is the authoritative source and is immune to Address-Library ID renumbering; CK2 oracle remains a possible future validator for the 17E write path only. (e) the earlier Option C' framing ("static header whitelist + fail-closed") still holds structurally, but its whitelist source is now the registry dump (authoritative) rather than header inference (partial), and "Ref"-vs-FormID ambiguity is resolved by typeKind==9.
  - 17C-2 research (librarian multi-lens): built-in @librarian (CommonLibSF FormReference mechanism + documented class field layouts) + @librarian-beta (sfse / published-dump search / xEdit known-unmapped reconciliation). Facts: FormReference/FormType are `BSReflection::Metadata::*` attributes (FormReferenceAttribute=BaseStringAttribute holding target class name; FormTypeAttribute=byte); BSResource::ID = asset resource hash (NOT a FormID); BGSAudio::WwiseGUID = 128-bit Wwise GUID (NOT a FormID); no public BSReflection.json dump exists (so our live capture is the authoritative source). xEdit known-unmapped Starfield FormID subrecords (IPDS, SCEN.SCEP, KSSM.KNAM, STND.NNAM, SFPT, SFTR) are PLAIN subrecord fields, not reflection-stream fields.
  - v7 outcome (2026-07-08, log `refl-probe/v7-run1.log`, dump `refl-probe/reflection-formref.json`): the 42 kind==9 form-reference fields across all 669 classes were captured with {class, field, offset} — this is the remap-critical "which slots are FormIDs" data and is SUFFICIENT for safe copy + remap. HOWEVER the finer detail "which exact record type each FormID points to" (target FormType per field, e.g. KYWD vs IMGS vs LIGH) was NOT recovered — the kind==9 pointer type node does not carry the target ClassType in the probed offsets and the FormReference target-name string is not resident. This is **editor-UI dropdown-filtering scope only and is explicitly DEFERRED** ("后续再说"). Safe copy + FormID remap do NOT need it. Resuming it needs the pointer-type-node layout reverse-engineered plus a live Starfield process (11-min SFSE relaunch). Starfield process closed 2026-07-08; no further live capture this session.

- Phase 17C-2 whitelist reconciliation + safe-copy design (2026-07-08, offline) is accepted as the grounded design basis for 17C-3 implementation. Three-lane oracle consult (built-in @oracle + @oracle-alpha gpt-5.5 + @oracle-beta opus) reviewed the design; both oracle blocking concerns were resolved by an offline reconciliation of the 211 real stream-classes (17A `refl-survey/classes-inventory.json`) against the 669-class registry dump. Artifacts: `whitelist/reconcile.py`, `whitelist/stream-class-safety.json`, design writeup `multi-lens/safe-copy-design-2026-07-08.md`.
  - CROSS-VALIDATION PROOF (load-bearing): for every registry FormID-bearing class (typeKind==9), 17A shows those exact fields as serialized wire-type "Ref" with UInt32 count 0, and per-class counts correspond (BGSWeatherSettingsForm 10==10, BGSAtmosphere::MiscSettings 3==3, etc.). So game form-references serialize as the "Ref" wire-type, NOT as "UInt32" — this CORRECTS the earlier option-c-tooling-verdict assumption ("FormIDs serialize as UInt32"). "UInt32" (43,783 slots) is plain integer data. This also resolved @oracle-beta's Ref-vs-kind9 cross-check concern.
  - ARCHITECTURE (chosen over the oracle-recommended class whitelist): safe-copy predicate = "the decoded reflection CLAS declares NO field of serialized type 'Ref'". Any Ref (or any decode failure) => unsafe/fail-closed. This is registry-INDEPENDENT (sidesteps the oracle-flagged registry incompleteness — the whole Houdini family is absent from the 669 dump), version-robust, fail-safe, and lower-intrusion (no 183-class table + version guard to maintain). Strictly more conservative than a whitelist; relaxes to "Ref => remap" when the remap slice lands.
  - ISSUE #5 CONFIRMED unblockable: `JaffaIIPlanetData [PNDT:0005E20A]` PCCC stream = HoudiniDataComponent + BSHoudini::HoudiniAssetData (both no-Ref; fields UInt8/String/List; payload two LIST chunks). The Houdini `Parameter` class (the only Houdini class with a Ref) is NOT instantiated (empty ParametersA) — ZERO Ref fields — so JaffaII copies verbatim with no dependence on the Houdini `Parameter.Value` Ref semantics. (Across all 2742 PNDT records, 1966 Ref occurrences exist, ALL from `Parameter.Value`; Parameter-bearing planets are conservatively blocked and deferred.)
  - IMPLEMENTATION (oracle-converged, 11 edit sites): new `ContainsUnsafeReflection` predicate (TwbElement/TwbContainer, mirroring ContainsReflection recursion; walks decoded CLAS Field Types for the "Ref" sentinel; fail-closed on decode issues); swap ContainsReflection->ContainsUnsafeReflection RedPill-aware at 5 gate sites (wbImplementation.pas 16851/18812/18943, xeMainForm.pas 15747, xeAutomationCommandsRecords.pas ~1034); and — THE #1 landmine per oracle-alpha — condition dfDontAssign/dfInternalEditOnly on `not wbRedPill` for wbREFL/wbRDIF/wbREFLStruct (6 flag edits) because those flags inherit DOWN via _DefFlagsInheritUp and would otherwise silently copy safe streams as EMPTY. Do NOT touch ContainsUnmappedFormID / master-first gate 16859, and do NOT wrap the Assign at 16926 in wbBeginInternalEdit. Full acceptance matrix in the design writeup. Ready for 17C-3 implementation.

- Phase 17C-3 IMPLEMENTED + built + deployed (2026-07-08), commits `1b36ad49` (impl + rename) and `d837c932` (oracle-beta review fixes). Delivered: content-aware `ContainsUnsafeReflection` predicate (Ref-field CLAS walk, fail-closed) + 5 RedPill-aware gate conditionings + 6 dfDontAssign/dfInternalEditOnly flag conditionings; renamed `wbStarfieldIsABugInfestedHellhole` -> `wbStarfieldReverseEngineeringIncomplete` (28 refs). Chose the Ref-field predicate over the oracle-recommended class whitelist (registry-independent, version-robust, fail-safe). Oracle-beta implementation review: no CRITICAL/HIGH; 2 MED fail-closed asymmetries (nil Fields, non-ordinal Field Type) + 3 LOW fixed in the follow-up commit. Build+deploy: LiteDebug Win64 (`bds.exe -b`, dproj default platform Win32->Win64 then restored) -> `Build\xEdit.exe` (PE x64, 55 MB) copied SHA256-identical to `D:\Starfield MO2\tools\xEdit\SF1Edit64.exe`. Starfield MO2 (PID 46560) stays online; no game restart.
  - Established Starfield daemon harness (for 17E): `.opencode/artifacts/phase16-.../scripts/run-runtime.ps1`. Launch `ModOrganizer.exe run -e SF1Edit64 -a "-SF1 -automation-serve -IKnowWhatImDoing -ItJustWorksTM -ThisIsFine -GiveMeTheRedPill"` (no --multiple on this MO2 2.5.3; routes through the standing IPC under active profile BB84); out-of-process call via `-automation-call-pid/-request/-response`; ping-poll readiness (first cache build ~11 min); session.save + CloseMainWindow shutdown.
  - 17E runtime semantic acceptance is the remaining step (NOT yet run): RedPill JaffaII PNDT [0005E20A] copy from cryomancer_z.esm -> byte-identity of the copied PCCC reflection payload + save/reload readback; non-RedPill same copy still blocked (upstream); a Ref-bearing record blocked with the new reason; malformed CLAS blocked. Cover oracle-beta 17E residual risks (all six reflection sigs, empty-CLAS, copy_into routing through CopyMainRecord, lazy-decode). In-game planet UI stays user-run.
- Phase 17E RUNTIME ACCEPTANCE PASSED + Fix A landed (2026-07-08), commit `e96c6d31`. The 17E first run (`runs/run-20260708-135614`, HISTORICAL/SUPERSEDED) proved the copy is mechanically correct (PCCC present + non-empty, correct masters, clean save, Houdini OBJT/LIST blob 100% byte-preserved) BUT surfaced a strict-byte defect: the copied PCCC was exactly +1 byte (54789 vs source 54788), a single NUL inserted between the STRT payload and the TYPE chunk. Fix A resolves it; accepted run is `runs/run-20260708-175107` with status=passed, 29/29, incl. decisive `pccc-byte-identity` (copied PCCC SHA256 == source SHA256, both 54788) and `pccc-size-match`.
  - DELIVERED (Fix A): new def flag `dfNoStringListTerminator` (wbInterface.pas, appended at enum END to preserve ordinals) gating the 3 synthetic-terminator sites in wbImplementation.pas (ArrayDoInit read-consume ~21936 + create ~21969, TwbArray.CheckTerminator ~22257); flag set ONLY on the shared `wbREFLSTRT` Strings array (wbDefinitionsSF1.pas ~5844), covering all 6 reflection sigs (REFL/RDIF/PCCC/PTCL/PSDF/XNSE). Build: LiteDebug Win64 clean (0 err, 27:44 cold), `Build\xEdit.exe` (55444298 B) SHA256-synced to `D:\Starfield MO2\tools\xEdit\SF1Edit64.exe`; dproj default platform Win32->Win64 then restored. Oracle review (built-in @oracle over the diff + empirical results): SHIP-WITH-NITS, no must-fix.
  - NEWLY KNOWN: (1) the +1 NUL is a PRE-EXISTING upstream behavior, NOT introduced by 17C-3 - xEdit's generic array machinery unconditionally appends a synthetic `TwbStringListTerminator` (1 NUL on save) after ANY variable-size dtString array; correct for empty-string-terminated FormID string lists, WRONG for the reflection STRT pool which is bounded by its sibling `Data Size` field + the next-chunk `TYPE` peek ($45505954 via SetShouldInclude), and whose `Data Size` (a static wbInteger) is not recomputed, so the phantom NUL made the stream internally inconsistent. "Pre-existing" != "game-tolerated": untouched records raw-pass on save, so reflection reserialization was uncommon under the gates. (2) The PCCC/Houdini reflection stream is NOT a top-level PNDT child - it nests under `Base Form Components`; the authoritative reflection presence/integrity signal is the raw PCCC byte extract, not an element-tree shortName match. (3) Daemon schema (grounded from captured responses): `records.find_by_form_id` -> `result.hits[0].object.{editorId,signature}`; `files.get_header` -> `result.masters` is a plain string array; `elements.children` -> `result.children[].object.shortName`; `records.copy_into` -> `result.record.formId`. The 17E harness (`scripts/run-17e-reflection-copy.ps1`, local-only) was corrected for these 5 paths, flipping the earlier 6 schema false-negatives to true PASS.
  - WHAT LATER PHASES MUST DO DIFFERENTLY: (a) The DECISIVE in-game gate (load the copied ESM, confirm JaffaII planet renders/loads clean) stays USER-RUN and is still pending - byte-identity is strong evidence but not the final semantic proof; do not claim issue #5 fully closed until the user confirms in-game. (b) Deferred 17E-e negative/coverage matrix remains in scope (own follow-up round): non-RedPill copy blocked, Ref-bearing record blocked with the new reason, malformed CLAS blocked, and all-six-reflection-signature positive coverage; per repo NOT-COVERED/PENDING-USER-REPORT discipline, only mark PENDING-USER-REPORT the cases that truly lack a fixture. (c) Upstream-PR prep (when it happens): strip local `Phase 17E` labels from the 4 new comments, and IF the flag is ever applied to a subrecord-level (non-nested) string array, also gate `TwbSubRecord.CheckTerminator` (wbImplementation.pas ~15513) - currently a no-op because STRT Strings is a nested TwbArray, deliberately not added to keep committed source == the empirically-verified binary.
- Phase 17E DECISIVE IN-GAME GATE PASSED (user-run, 2026-07-08). JaffaII was not reachable this session (in-game ship fuel limit), so the positive gate was run on a reachable planet: Jemison `JemisonPlanetData [PNDT:0003F5A1]`. Method (all via the Fix A SF1Edit64 daemon, RedPill): created `Jemison_P17E_Patch.esm` (masters Starfield.esm + cryomancer_z.esm), `records.copy_into` Jemison from cryomancer_z.esm's WINNING override as a deep override (this is the exact issue #5 copy-from-override scenario, and it re-exercises Fix A's reflection copy), then edited the sibling non-reflection field `TEMP - Temperature in C` 21 -> 50 via `elements.set_native_value [8]` (readback confirmed 50.0), then `session.save` (dirty cleared, on-disk `overwrite\Jemison_P17E_Patch.esm`). USER RESULT in-game standing on Jemison: temperature reads 50 C (patch active + copy/edit/save produced a live engine-valid ESM), AND the planet's biomes, POIs, and terrain all render and load normally. Because the entire procedural surface is GENERATED FROM the copied PCCC reflection stream, correct surface generation is direct proof the reflection copy is byte-faithful and engine-valid - the strongest reflection-integrity test available, and it PASSED. Earlier same-day the byte-identical JaffaII override also loaded into a live save without crashing.
  - DELIVERED: issue #5 core capability - safe byte-exact Starfield reflection copy - is now proven END-TO-END: mechanical copy -> byte-exact save (JaffaII SHA256 == source) -> in-game load -> correct reflection-driven surface (Jemison). Edit-and-save (not just copy) is also proven: editing a sibling subrecord and saving left the reflection stream intact and game-valid.
  - NEWLY KNOWN: (1) copy-from-override works cleanly (cryomancer_z's Jemison override deep-copied, masters resolved to [Starfield.esm, cryomancer_z.esm]). (2) Editing a sibling non-reflection subrecord (TEMP) and saving does NOT disturb the reflection PCCC - Fix A holds through a real edit-and-save cycle. (3) `extract-pccc.py`'s raw low-24 formId scan can collide on override-heavy plugins (querying 0003F5A1 in cryomancer_z.esm grabbed a local `RFGP:0x0103F5A1` whose low-24 matched, not the PNDT); the daemon resolves records correctly, but that raw extractor needs a signature filter before trusting its source-side output. The in-game surface render SUPERSEDED the need for a source byte-comparison (per this fork's Starfield evidence standard, working in-game behavior is the stronger evidence).
  - WHAT LATER PHASES MUST DO DIFFERENTLY: (a) The issue #5 POSITIVE path is now CLOSED / in-game validated - no longer pending user report. (b) RELEASE IS DEFERRED by user instruction: do local closeout + commit + push only, no version bump / tag / GitHub release this round. (c) Still deferred (own follow-up round): 17E-e negative/coverage matrix (non-RedPill blocked, Ref-bearing blocked, malformed CLAS blocked, six-signature coverage) and the upstream-PR comment/guard nits. (d) The user's real issue #5 goal (JaffaII EOVR / Edited-Worldspaces merge) is now unblocked at the capability level. Test ESMs were deleted by the user post-test; Jemison patch harness + request/response evidence preserved under `.opencode/artifacts/issue5-jemison-patch/`.
- Starfield schema-load cleanup slice (2026-07-08/09) is runtime verified for its targeted families. Delivered: `ASPC` now accepts observed `OPDS` after `ODTY`; `PERK` now accepts observed `PRKC`/`CTDA` condition payload runs plus `CIS1`/`CIS2`, opaque `CNDD`/`CIES`, in-rank `PRUC`/`PRRF`, and a post-rank `PRUC`/`PRRF` tail variant; `LGDI` accepts observed `MNAM` second base-list pointer, `LNAM`, repeated `CITC`/`CTDA` condition groups, and opaque `FNAM/KNAM/GNAM/HNAM/JNAM` tails; `GWED` is defined as an `EDID` + `REFL` record and added to SF1 group order. Evidence: raw read-only layout captures and tripwire scripts under `.opencode/artifacts/starfield-schema-load-cleanup/`, source check `schema_source_checks.py --families ASPC,PERK,LGDI,GWED,RESIDUAL` RED-before/GREEN-after, canonical `bds.exe -b xEdit.dproj` transcript `xEdit.err` showing `Building xEdit.dproj (LiteDebug, Win64)` / `Success`, final SF1Edit64 SHA256 `CF411627256523B6266CA439F140E6A26C589E7EA61B73481A112E7524319A47`, and runtime run `.opencode/artifacts/starfield-runtime-loadlog-verify/runs/run-20260709-071757/` showing daemon ready/clean close with `PERK` errors = 0, `LGDI` errors = 0, `GWED` unknown = 0, and `ASPC` errors = 0.
  - Newly known: the supplied Starfield load-log's targeted schema families were real parser drift, not corrupt records. PERK drift was broader than the first `PRUC/PRRF` symptom: condition fragments are a repeated payload stream before `EPFT`, not a single structured condition block; some records also carry a post-rank `PRUC/PRRF` pair. `SFBGS00D.esm`'s `GWED` is a small REFL-bearing top-level record. LGDI's newer condition blocks reuse known `CITC`/`CTDA` primitives while the equipment/category tails remain intentionally opaque until CK/community semantics are recovered.
  - Later phases must do differently: Step 2 is now unblocked for focused reverse-engineering. Keep `WEAP BGSQualityUpgrade_Component` as a focused base-form-component decode slice, keep the `ronin`/other FULL failures in the UTF-8/string-decoding lane, and treat the `EnhancedLightsandFX` unresolved base reference as mod/load-order data unless fixture evidence proves otherwise. Phase 17 v7 reflection FormID remap stays separate from this load-cleanup slice.

- Starfield Step 2 initial reverse-engineering adjudication (2026-07-11) completed as a report-only round. Delivered: four lane reports under `.opencode/artifacts/starfield-step2-re/reports/` and four reviewed TDD plans under `docs/plans/2026-07-11-starfield-step2{a,b,c,d}-*.md`; Quality Upgrade locks `BFCB` string + headerless ordered LE-`u32` FormID `QUPA` + empty `BFCE`; PERK locks canonical 32-byte CTDA conditional FormID handling plus PRKE Type 1 Ability SPEL; Legendary Item locks the known typed-tail strides while retaining the unresolved anchor gate. The frozen evidence validator remains clean: 27 captures, 1317 validated subrecords, 0 failures. The model scorecards are evidence-consistency/risk rubrics only, not objective format-accuracy percentages; the earlier `72.82% retained capability` interpretation is superseded and must not be reused.
  - Reflection deep RE reopened and closed (2026-07-11): two independent investigators used the same frozen prompt with prior model reports/scores hidden (Opus 4.8 and GPT-5.6-sol xhigh; DeepSeek excluded), followed by a separately authored main-session parser and recursive semantic walker. Final report: `.opencode/artifacts/starfield-reflection-deep-re/final-report.md`. The main validator independently reproduced the eight-file current corpus totals (47,017 streams, 2,107 populated MAPC, 1,135 DIFF, zero semantic decode errors) and then decoded the expanded 366-ESM corpus (66,751 streams, 2,544 populated MAPC, 1,924 DIFF, 10,793 USER, 1,193 USRD, zero semantic decode errors). Version-4 OBJT/LIST/populated-MAPC/sparse-DIFF/deep sibling recursion and tagged Ref unions are now byte-grounded. USER/USRD is User-bit driven but class-specific: BSFloatCurve and MaterialProperty Entry use owner+actual indexes with child-count trailers 1/2; WwiseSoundHook uses owner plus tagged leaves and trailer 0. The all-UInt32 scan found the same 21 ordinary class/field identities in both corpora; only `NavMeshSplineExtraData::ChunkDataRef.RefID` is a raw FormID (current 28,114/28,114 -> REFR; expanded 32,195 resolved targets, all REFR). Small integer/hash/color/index fields produce coincidental low-FormID hits and remain scalar. Five registry-only AssetDiagnostics FormId-named fields appeared in zero wire streams across all 366 ESMs and remain unknown-future fail-closed cases, not current remap blockers.
  - Later phases must do differently: the revised Reflection plan now enables DIFF, populated MAPC, deep recursion, owned pointers, and the three observed USER/USRD serializers; do not retain the old fixture-only/deferred-DIFF boundary. The raw-UInt32 allowlist is exactly `ChunkDataRef.RefID`; unknown chunks/classes/sentinels/User serializers or a new raw-UInt32 identity fail closed before mutation. No Starfield launch is needed for implementation or further reverse engineering: a main-menu launch would only repeat the existing registry and cannot create missing wire bytes. Starfield is reserved for post-implementation cross-module copy/master-add/remap/save/reload semantic acceptance. Execute the four Step 2 implementation plans separately; preserve the Win64 binary before the Win32 steady-state build, restore dproj bytes by hash, honor process-lifecycle permission boundaries, and require in-game verification only for engine-visible semantic claims.
  - Current execution position (2026-07-11): the runtime-verified ASPC/PERK/LGDI/GWED schema-load cleanup was independently reviewed, rechecked against the direct MNAM/CNDD/CIES captures, and preserved as bounded source commit `368539ab`. Fresh closeout gates passed: schema source checks GREEN, 27 captures / 1317 payload entries / 0 evidence failures, eight source ESM hash pairs unchanged, and final runtime-log readback still reports zero PERK/LGDI/ASPC/GWED target errors. The clean source baseline now advances to the QUPA RED contract. QUPA is the smallest coherent ready slice and closes the remaining `BGSQualityUpgrade_Component` parser gap; this is an explicit execution choice, not a dependency implied by the old A/B/C/D labels. Historical July 9 design/execution docs are marked completed/superseded; the four July 11 TDD plans are the active controllers. Starfield stays closed until QUPA has production code, dual-platform builds, and a runtime acceptance package ready.
...
- Phase 11 locally adds `session.navigate_to_record` as a record-root navigation primitive and verifies it against the MO2-managed xEdit target.
...
- Phase 13 fresh LiteDebug builds were taken for both targets: `Build/xEdit.exe` (Win32) and `Build/xEdit64.exe` (Win64), both `Success`. Evidence is `.opencode/artifacts/phase13-elements-mutation/build-t10/`. Runtime semantic E2E ran against the then-current MO2-managed xEdit tool target via the legacy portable-harness bootstrap. Final accepted run after the 2026-06-08 fixture rework and AC-001 drop is `.opencode/artifacts/phase13-elements-mutation/runs/run-t12-accepted/` with `status:"passed"`, `caseCount:30`, `passed:28`, `failed:0`, `notCovered:2`, `harnessFailure:null` (supersedes `run-t11-accepted/` which superseded `run-t10-accepted/`; the t11 snapshot retains the 14-candidate AC-001 probe evidence). Pre-shutdown `session.save` persisted `opencodetest.esp` via `savedFilesPendingShutdown`. Semantic acceptance note: `.opencode/artifacts/phase13-elements-mutation/summary/semantic-acceptance-phase13.md`.
...
- This machine now has a documented local verification path:
  ...
  - runtime verification must use the WastelandBlues Fallout 4 harness; the canonical xEdit target is `B:\WastelandBlues 2.0\Stock Game\Fallout 4\Tools\FO4Edit\FO4Edit.exe`, and syncing the fresh LiteDebug build into that tool path is explicitly allowed
  - the Stock Game prohibition is specifically against mutating `B:\WastelandBlues 2.0\Stock Game\Fallout 4\Data`; `D:\TES5Edit-contrib\Build\xEdit.exe` is not an acceptable runtime target for this FO4 MO2-backed verification path
  - the canonical entrypoint is `D:\awesome-bgs-mod-master\tools\mo2-vfs-launcher\xedit-client.ps1 process launch`, which delegates to the Mo2AgentControl broker inside the running WastelandBlues MO2 GUI
  - the drift to avoid is MO2 CLI `run` mode, direct Steam Data discovery, or any launch path that bypasses the broker client
...
- The local MO2 harness note is now sharper:
  - the valid entrypoint is the broker-backed `xedit-client.ps1 process launch` command documented in `AGENTS.md`
  - the drift to avoid is MO2 CLI `run` mode or any launch that bypasses the in-GUI broker
  - `B:\WastelandBlues 2.0\Stock Game\Fallout 4\Data\...` is not acceptable as disposable workspace state; future local runtime work must not write game Data there
  - if an agent believes it needs to change game-local Data files, it must package those changes as a dedicated MO2 mod under `B:\WastelandBlues 2.0\mods\<mod-name>\...` and let MO2 VFS project them into runtime instead of mutating the game Data tree directly
- Phase 6A serve-startup follow-up has now completed the explicit-module and programmable-wrapper implementation attempt:
  ...
  - historical/superseded: the local MO2 harness used the legacy programmable bootstrap with a no-spaces wrapper target that wrote the actual xEdit child PID and launched with `-FO4`, `-automation-serve`, and the profile `plugins.txt`; the earlier positional `Fallout4.esm` argument was removed after tracing it to xEdit's real-file positional module validation path
  ...
  - historical/superseded: an earlier diagnostic physically mirrored official FO4 masters into the Stock Game `Data` directory and then switched `run-phase6A-verification.ps1` back to the programmable wrapper route for PID/profile control; this is no longer an acceptable harness direction under the no-Stock-Game-Data rule, and future debugging must use the WastelandBlues `FO4Edit.exe` target plus MO2 overlay/overwrite state instead of Stock Game Data writes
  - restored after review and later tightened: the current harness target is now the MO2-managed `B:\WastelandBlues 2.0\Stock Game\Fallout 4\Tools\FO4Edit\FO4Edit.exe` for daemon and call-mode execution, and fresh LiteDebug builds may be synced into that tool path; all fixture/proof writes still go to MO2 `overwrite`, and no `Stock Game\Fallout 4\Data` copy/sync/delete step is allowed
  ...
  - the next bounded harness step replaced the earlier dedicated-overlay expectation with a real MO2 `overwrite` proof: `files.create` plus `session.save` for `VT_Phase6A_OverlayProof.esp` must create `B:\WastelandBlues 2.0\overwrite\VT_Phase6A_OverlayProof.esp` and must not create `B:\WastelandBlues 2.0\Stock Game\Fallout 4\Data\VT_Phase6A_OverlayProof.esp`
...
- Phase 6E Task 6 contract hardening is now in place locally and has been included in the Task 7 final local-only closeout:
  ...
  - historical local wrapper examples are preserved under `.opencode/artifacts/phase6E-contract-hardening/examples/` for the legacy MO2 bootstrap, `jobs.start/get/findings` polling, explicit `session.save`, and artifact-local restart plugin lists
...
- Historical/superseded lesson: the former MO2 harness needed a sharp distinction between its CLI bootstrap entry and the launcher implementation body after MO2 activation. The WastelandBlues runtime replaces both with the in-GUI broker client.
- Historical/superseded lesson: the former drift came from bypassing the legacy MO2 bootstrap and running its implementation body directly from the host shell. Current runs must not bypass the Mo2AgentControl broker client.
...
- Historical/superseded: the programmable wrapper path previously looked like the better mainline MO2 candidate after official masters were mirrored into the Stock Game `Data` path because it gave explicit child-PID handoff and profile `plugins.txt` control. That candidate direction must not be reused as a reason to write to Stock Game Data; the current harness syncs the fresh build into the WastelandBlues `FO4Edit.exe` tool path, launches that canonical target through the broker client, and verifies fixture persistence through MO2 `overwrite` without copying, deleting, or mutating anything under `Stock Game\Fallout 4\Data`.
- Historical/superseded by the accepted 6A baseline: the Phase 6A harness had drifted beyond the documented Phase 5 contract by inserting a PowerShell wrapper and harness-side plugin-list/module arguments; restoring the legacy bootstrap to target xEdit directly did not by itself restore named-pipe readiness during that diagnostic pass, but the final accepted 6A harness later reached the required MO2-backed daemon readiness.
...
- We now know canonical tool-target discipline is a first-class verification requirement: final 6B-E runtime evidence is only meaningful when the fresh LiteDebug binary is synced to and launched as the WastelandBlues `FO4Edit.exe` tool target, not from the repo `Build\xEdit.exe` or a launch path that bypasses the broker client.
...
- Later runtime verification should preserve the current harness boundary: sync the fresh LiteDebug binary into the WastelandBlues `FO4Edit.exe` tool path, keep MO2 running, then let the Mo2AgentControl broker client start that canonical xEdit target under VFS. Do not bypass MO2, do not point Phase 6 FO4 verification at `D:\TES5Edit-contrib\Build\xEdit.exe`, and do not use MO2 CLI `run` mode when the goal is canonical verification.
...
- Later runtime verification must keep the canonical MO2/LiteDebug path as a non-regression gate: fresh `LiteDebug` build evidence, sync to `Stock Game\Fallout 4\Tools\FO4Edit\FO4Edit.exe`, broker-client launch, pipe readiness, MO2 `overwrite`/overlay fixture writes, and no Stock Game `Data` mutation.
...
- Clarified the local MO2 harness contract: canonical verification enters through `xedit-client.ps1 process launch` and the in-GUI Mo2AgentControl broker, not through MO2 CLI `run` mode.
...
- Earlier unknown: was the MO2 harness drift about using the legacy programmable bootstrap at all?
...
- Newly learned harness/lifecycle constraint: target-model selftests that need loaded records cannot run immediately after `Application.CreateForm`; they must wait for `WMUserLoaderDone` after `TLoaderThread` populates `wbModulesByLoadOrder`, or valid locators falsely fail as missing files. The final historical harness also avoided passing `-P:<plugins.txt>` and extra positional module arguments through its legacy bootstrap; current runs use the broker client's session plugins file contract.
...
- Daemon entrypoint: `xedit-client.ps1 process launch` through the running WastelandBlues MO2 GUI broker.
...
- Live-witnessed C3 recovery and 8D fresh-final-binary GUI Apply Script success on the then-canonical MO2-managed xEdit target. `gui-non-regression-001` (carried as not-covered from Phase 7C) is now retired with operator-evidenced proof.
...
Status: local closeout accepted at `1.0` scope for the xEdit-side automation surface only. Phase 10 final report is `.opencode/artifacts/phase10-semantic-e2e/report/phase10-final-report.md`. The closeout build hash matched the then-canonical MO2 tool target: `4ADCF3350E083A9635926C939F812176FA0983C3B4E0F9EBCE042E03766FF8A2` in both `Build\xEdit.exe` and the historical runtime executable (`.opencode/artifacts/phase10-semantic-e2e/report/closeout-binary-sync.md`).
...
- Verified the positive case against the then-canonical MO2-managed runtime target, launched with `-FO4`, `-IKnowWhatImDoing`, and `-automation-serve`; current runs use the WastelandBlues `FO4Edit.exe` target and broker client documented above.
...
## Step 2A QUPA — malformed-diagnostic decision RESOLVED (2026-07-12, delegated by user)
- Decision (owned by maintainer-agent per user delegation): KEEP fail-closed-to-0 on non-4-aligned QUPA. Rejected floor(n/4) because a non-aligned payload is corrupt at an unknown offset and floor(n/4) risks presenting misaligned garbage FormIDs as valid; count 0 never shows garbage. Non-aligned QUPA is external-corruption only (xEdit always writes 4-aligned), so a runtime Check is disproportionate hardening for a non-occurring case. Documented the deliberate choice + the optional ctCheck hardening path in code (commit 3020dac1). No behavior change; r2 binary stays valid.
- Step 2A QUPA is now CLOSED: typing + load-log + copy/record-remap + save/reload all oracle-ACCEPTED; entry-level remap = sound inference (WEAP reflection-blocked by design); malformed = safe fail-closed (decision recorded).

## deep-RE Step 2B — reflection FormID remap — CLOSED (ACCEPT-WITH-CAVEATS, 2026-07-13)
Delivered (directly witnessed): Starfield Ref-bearing reflection copy UNBLOCKED (WEAP 0200D2DB copyOk=true, hard-blocked in 2A; FFKW 00000AFA also copies). New wire walker (Core/wbInterface.pas TwbReflectionWalkContext + wbReflectionCollectFormIDSlots) decodes the full BETH v4 grammar (OBJT/DIFF/LIST/MAPC/nested/tagged-Ref/USER x3), locates every FormID slot, fails closed on unknown grammar. Decode-aware copy gate (wbImplementation.pas ContainsUnsafeReflection: rdsComplete=safe else unsafe) replaces the schema-only "any Ref=unsafe". Commits 9a4b2925/4f838143/7c849d16. Multi-lens design (3 models converged), independent static review PASS, LiteDebug Win64/Win32 clean.
Now known: (a) walker runtime integration works for the WEAP Base-Form-Component placement (ContainingSubRecord yields the BETH stream). (b) Offline: the walker's oracle mirror decodes all 47,017 + 66,751 real corpus streams COMPLETE, zero errors, raw-UInt32 allowlist = exactly {NavMeshSplineExtraData::ChunkDataRef.RefID}. (c) The WEAP copy unblock also delivered the 2A NOT-COVERED item: QUPA entry-level remap directly witnessed (6 targets, 02->01 file-local rebase, load-order preserved).
HONEST BOUNDARY (semantic review, load-bearing): the reflection-FormID VALUE-rebase is NOT directly witnessed — every witnessed reflection Ref was no-op-equivalent (WEAP pParentForm null; FFKW into Starfield master-0), so byte-identical output cannot distinguish a working reflection rebase from an untouched slot. QUPA proves the native callee; static review proves the walker mirrors the oracle; but the walker's runtime handoff on a value-changing reflection Ref is INFERRED, not witnessed.
REQUIRED FOLLOW-UP (owned, next daemon excursion, NOT "later"): build a non-0-master reflection Ref fixture with a VISIBLE file-local rebase, read back at OBJT byte level (recipe in .opencode/artifacts/starfield-step2b-reflection/summary/final.md). Until GREEN, general reflection remap is mechanism-inferred, not semantically proven. Claim wording downgraded accordingly.
Later phases: 2B's grammar walker + decode-aware gate pattern is the template for 2C (PERK conditions) and 2D (LGDI tails) if those also carry embedded FormIDs needing remap. Do NOT reuse "copy succeeded + bytes identical" as remap proof for a null/master-0 Ref — require a value-changing witness.

## deep-RE Step 2B - CRITICAL FOLLOW-UP: reflection FormID remap is BROKEN (2026-07-13)
Direct-witness fixture (user-requested) DISPROVED the inferred remap. Copying a self-referencing reflection record from SFBGS050.esm (self-index 02) to a plugin where SFBGS050 is master index 01: the record's own FormID rebases 02->01 correctly, but the REFLECTION Ref is written VERBATIM (stays 02) -> corrupt cross-plugin reference. Confirmed on-disk on 2 records (WEAP BreechBlock refl 020A9058; ACTI refl 0207FA91). The walker LOCATES + DECODES reflection refs (Task 4 proven) but TwbReflectionPayloadDef.CompareExchangeFormID does NOT fire during records.copy_into. Normal internal refs (QUPA) DO rebase in the same copy, so the copy-remap path skips reflection payload leaves specifically.
STATUS CORRECTION: Step 2B delivered reflection COPY-UNBLOCK + correct slot LOCATION/DECODE + gate + corpus conformance, but reflection REMAP is DEFECTIVE (copies produce corrupt refs). The earlier "remap mechanism inferred" is disproven. This must be fixed before reflection copy is safe; until then the decode-aware gate arguably allows an unsafe copy (corrupt refs) that the old fail-closed gate blocked.
REQUIRED NEXT: root-cause why CompareExchangeFormID/BuildRef on the reflection payload leaf is not exercised by copy_into remap; fix; re-witness with the self-ref fixture (regression test); reconsider whether the gate should stay fail-closed until remap is proven.

## deep-RE Step 2B - REMAP FIXED + WITNESSED (2026-07-13)
The reflection-remap bug is fixed and directly witnessed. Root cause: records.copy_into adds masters before the copy, so the file-wide MastersUpdated never touches the new record; the copy runs through value-def Assign, and TwbReflectionPayloadDef inherited the byte-array verbatim Assign (no FormID translation) while wbFormID has a translating Assign (why QUPA rebased). Fix (commit 2536d543): TwbReflectionPayloadDef.Assign copies the bytes, decodes the stable source leaf to locate slots, then translates each reflection FormID slot source-file->load-order->target-file exactly like wbFormID. On-disk witness on 2 self-ref records (WEAP+ACTI, SFBGS050->VTOS): reflection refs rebase 02->01 correctly, load-order preserved; record FormIDs still correct. STATUS: Step 2B reflection copy+remap is now directly witnessed end-to-end (copy-unblock + slot location/decode + gate + corpus conformance + FormID remap). Latent follow-up (not the copy path): the walker's MastersUpdated/CompareExchangeFormID re-decode + exact-pointer Project is fragile on modified records, affecting in-file master REORDERING of reflection records; fix = mode+length region match + re-fetch live pointer. Tracked.

## deep-RE Step 2B - Lane A reorder fragility: EMPIRICALLY NOT REACHABLE, speculative fix REVERTED (2026-07-13)
The "latent follow-up" tracked in the entry above was investigated to closure this round.
Method: 3-lens oracle consult SPLIT (built-in @oracle = ALREADY-CORRECT; @oracle-alpha + @oracle-beta = REACHABLE-FAILURE). Ground-truth code check confirmed the dissenters' MECHANISM in the abstract: TwbStructDef / TwbArrayDef carry NO lifecycle overrides and the base TwbValueDef.MastersUpdated/BuildRef/CompareExchangeFormID/FindUsedMasters are literal no-ops, so the nested reflection leaf is reached ONLY via the element-tree descent (TwbValue.MastersUpdated) carrying a by-value pointer. On that basis a speculative "keep exact Project, re-fetch the live leaf pointer" fix was implemented + built (commit 1ff75a86, LiteDebug Win64 `19837918...`).
DISPROVEN AT RUNTIME: RED reproduction on the buggy pre-Lane-A binary (remapfix `FC10E1E0`) FAILED in every construction. 4 automation scenarios (copy->masterop; copy->immediate masterop; copy->set_value-detach->masterop; copy->read->set_value-detach->masterop) AND a DEFINITIVE in-process Pascal script (ElementByPath->SetEditValue detach->AddMasterIfMissing sort, consecutive statements with ZERO command boundary / zero intervening settle) all remapped the reflection ref CORRECTLY 01->02 when SFBGS050 shifted index. The stale-by-value-pointer state never survives into the reflection override.
ROOT CAUSE the bug does not fire: TwbSubRecord.MastersUpdated (wbImplementation.pas:16163) runs DoInit(False) and re-fetches GetDataBasePtr (UpdateStorageFromElements re-merges detached child storage into a contiguous buffer) BEFORE the element-tree descent reaches the reflection leaf, so the leaf's pointer is fresh when TwbValue.MastersUpdated captures it and the exact-pointer Project passes on the UNFIXED binary. @oracle lens 1 (ALREADY-CORRECT) was empirically right; the 2:1 majority and this repo's own prior "reachable latent bug" note were wrong.
DECISION: fix REVERTED (commit 001b4571) per YAGNI / no reachable bug / unverifiable-via-RED. Evidence: `.opencode/artifacts/starfield-step2b-lane-a/RED-reproduction-findings.md` (all four automation runs + the in-process script transcript). Runtime state left clean: SF1Edit64.exe restored to pristine `CF411627`, owned daemon closed gracefully, junk fixture ESMs removed from overwrite, Starfield MO2 not disturbed, Steam Data untouched.
Later phases: Step 2B is now FULLY CLOSED with no Lane A debt (copy remap on-disk witnessed; master-op / sort-masters remap empirically correct on the unmodified product code). Do NOT re-add live-pointer-refetch defensive code without a reachable reproduction. The exact-pointer Project in the reflection walker relies on the DoInit-settle invariant (leaf is always settled into the contiguous subrecord buffer before any lifecycle override observes it); a hypothetical future caller that dispatches a lifecycle override directly on a detached materialized reflection leaf, bypassing the subrecord DoInit, is the only theoretical break and is not created by any current automation or in-process-script path. 2C (PERK conditions) and 2D (LGDI tails) remain the outstanding Step 2 slices; reuse 2B's grammar-walker + decode-aware-gate + translating-Assign pattern.

## deep-RE Step 2C + 2D - DELIVERED + SEMANTIC E2E WITNESSED (2026-07-13)
Both remaining Step 2 slices are code-complete, compile clean (LiteDebug/Win32+Win64), and semantically accepted on the live MO2/SF1Edit64 RedPill daemon. Oracle semantic audit: 2C PASS, 2D PASS on proven core, no must-fix defect. Commits on automation-4.1.6: 030d6b5d(2C), 4f7b5e4a(2D formatter, best-of-N winner alpha), f628c917(2D FromEditValue), 538baf4f(2D count-callback fix), 6ec71d77(2D GetIsEditable copy-fix). Not pushed. Build binary SHA 60D702A7.

DELIVERED (2C): PERK opaque CTDA/CIES -> reused canonical `wbConditionCTDA` member (one def, bound in both wbConditions and the PERK payload union; no PERK fork). Live read of SRB_BookPerk: conditions decode typed (HasPerk/Self, GetIsObjectType/Book, IsInList/FLST FormID x2), interleaved PRKC/CTDA parsed. Condition FormID remap rides the canonical param deciders.

DELIVERED (2D): LGDI FNAM/KNAM/JNAM 12-byte rows typed; word3 = conditional FormID via TwbConditionalFormIDByMasterObjectFormater, remapping ONLY for the pinned anchor (SFBGS00D.esm + objectID 033E3D = self-ref MISC Y2_X-Tech_Misc, name-checked via SameText so another master's 033E3D is never misclassified), raw non-remappable otherwise. MNAM->[LGDI] (was wrongly [LVLI]), ANAM->untyped wbFormID; typed LNAM/GNAM(+4)/HNAM(+8) arrays. On-disk witness (bytes parsed from written .esm): anchor stored 01033E3D at SFBGS00D full-slot1 and REBASED to 02033E3D at full-slot2 (both resolve to Y2_X-Tech_Misc); scalar 0x0000000F byte-identical in both; save+reload consistent.

NOW KNOWN - three runtime defects the static gates (source-check, corpus tests, LiteDebug compile, 3-oracle blind review) ALL missed and only live readback caught (validates good-dev-behavior rule 34; static gates are support signals, not acceptance):
1. Count callbacks returned 0 for every tail: wbLGDI8/12CountCallback used the wbTryGetContainerFromUnion+.Container.DataSize UNION-ARM DECIDER pattern (cf. wbBFCDATADecider) which does not resolve for plain non-union subrecord arrays -> tails collapsed to empty, anchor rows never exposed. Fix: wbQUPACountCallback span idiom (aEndPtr-aBasePtr) div stride, fail-closed. The plan prescribed the wrong idiom and all 3 best-of-N candidates inherited it.
2. copy_into zeroed every word3 (anchor AND scalar): the copy is gated by CanAssign -> GetIsEditable; the anchor-only GetIsEditable override evaluated IsCurrentAnchor against the TARGET leaf's SetToDefault(0) (not the source), returning False, so the correct Assign never ran. Fix: allow editability while wbCopyIsRunning>0 (a real maintained global) so Assign runs; GUI read-only for scalars preserved outside copy.
3. FromEditValue gap (caught pre-runtime by grounding SetEditValue): non-anchor writable via automation set_value hitting base FormID-string parse. Fix: gated FromEditValue.

GAP DISPOSITION (oracle-adjudicated): ACCEPT - MNAM/ANAM/GNAM/HNAM copy-remap + 2C condition remap on copy (standard wbFormID / canonical decider machinery, most-exercised path in xEdit). PENDING-USER-REPORT - malformed 10/14-byte fail-closed tail fixtures (no CLI path to inject sub-stride garbage; real data is always N*stride). UNEXERCISED-BUT-READ-CORRECT (logged so a future loop does not read "not witnessed" as "green"): (i) in-place MastersUpdated/IsOldAnchor master-reorder path (only the copy/Assign path was witnessed); (ii) FromEditValue set_value round-trip (positive+negative) and its StrToInt64 EConvertError surfacing as clean invalid_request. Intended design boundary (do NOT "fix"): a scalar tag whose value exactly equals the pinned identity would be treated as the anchor - intrinsic to heuristic-free pinned identity, astronomically unlikely for small tags like 0x0F.

Later phases: Step 2 (2A QUPA + 2B reflection + 2C PERK + 2D LGDI) is now delivered and semantically witnessed on well-formed data. The reusable lesson: for any new callback-counted subrecord array, count from the aBasePtr/aEndPtr span (wbQUPACountCallback), NOT the union-decider container walk; and for any value-gated formatter, remember copy is gated through CanAssign->GetIsEditable against the target's default-0, so editability must be copy-aware (wbCopyIsRunning) or unconditional. Runtime state left clean: tool restored to pristine CF411627, daemon closed gracefully, throwaway ESMs removed from overwrite, Starfield MO2 (36472) untouched, Steam Data untouched.

## deep-RE Step 2D - residual arms closed (2026-07-14)
The three unexercised/PENDING items from the entry above were closed via a second live excursion (warm-cache daemon, build 60D702A7). No code changes were required.
- IN-PLACE MastersUpdated (arm i): created a target with SFBGS00D at full-slot1 (anchor stored 01033E3D), then ADDED ShatteredSpace as a master to shift SFBGS00D to full-slot2; the existing record's anchor rebased 01->02 ON-DISK via the file-wide MastersUpdated path (distinct from the copy/Assign path already witnessed), still resolving to Y2_X-Tech_Misc. WITNESSED.
- FromEditValue (arm ii): the non-anchor scalar is rejected at the automation surface with read_only_target (the daemon enforces GetIsEditable=false there), so a set_value cannot write it and the StrToInt64 raw arm is unreachable for scalars (value stays 0x0F). The editable anchor accepts a valid FormID edit value (round-trip, changed=false) via inherited FromEditValue; garbage input on the anchor is cleanly caught (internal_error "'$X' is not a valid integer value") and the daemon survives. CLOSED - no crash, no corruption.
- Malformed-tail fail-closed: binary-crafted a misaligned 14-byte FNAM (perturbed an active esm; FNAM size + LGDI dataSize + GRUP size reconciled). Result: xEdit does NOT load the plugin (absent from files.list though active in plugins.txt; no "GRUP Top LGDI processed" line for it) -- a NON-DESTRUCTIVE whole-plugin load failure. Because the count callback fails closed to 0 for ANY misalignment (mod stride != 0), this is representative of all malformed sizes. Bytes preserved (source untouched on disk -> no silent empty-array is displayed and no save-truncation is possible). SAFE for real always-aligned data; harsher than the old opaque wbByteArray (which loaded malformed data as raw bytes). Softening to a graceful inline error instead of a whole-plugin skip remains PENDING-USER-REPORT -- a design refinement, not a corruption risk.
Runtime state left clean again: tool restored CF411627, both daemons closed gracefully, throwaway ESMs + their plugins.txt entries removed, MO2 36472 untouched, Steam Data untouched.

## Phase 17 A/B acceptance + 17E-e decode-aware clarification (2026-07-14)
Formal runtime acceptance of the contract-0.22 bounded daemon-surface fixes (lane A) and the deferred 17E-e reflection negative/coverage matrix (lane B), run on RedPill + non-RedPill SF1Edit64 daemons (deployed r2 60D702A7) against the live BB84自用2 188-file Starfield profile. This round changed NO product source — it is verification only; lane D (upstream PR) is permanently dropped by user instruction. Evidence root `.opencode/artifacts/phase17-ab-acceptance/` (gitignored); working note `FINDINGS.md`.

Delivered / verdicts:
- A1 (0.22 locator omit-path) PASS: `records.copy_into` accepts `{file,formId}` locators with the `path` key OMITTED on both source and target (== `path:""`); KYWD SFBGS050:2D001570 copied ok. Source-confirmed: copy_into only rejects `Path<>''` (xeAutomationCommandsRecords.pas ~1107), so omitted path defaults to record-root.
- A2 reclassified: WTHR 0000015E carries NO reflection subrecord (plain FormID refs only); its ok copy is a plain non-reflection copy with no gate bearing. Not a reflection witness.
- A3 (0.22 unmapped-FormID denial) — mechanism + structural reachability + masterless-target all proven, end-to-end block witness NOT-COVERED / PENDING-USER-REPORT. Oracle ruling: the unmapped branch is RedPill dead code (`dfUnmappedFormID` is stripped at def-build because RedPill sets `wbStarfieldReverseEngineeringIncomplete:=False` before DefineSF1), and is only reachable on a non-RedPill daemon. On non-RedPill a masterless target IS buildable (`files.create initialMasters:[]` yields masters `[]`, satisfying the non-game-master-first condition), but no loaded record in the 188-file order has a POPULATED unmapped-FormID field (KSSM.KNAM / STND.NNAM / IPDS / SFPT / SFTR all unset; STND has no NNAM element at all), so the branch cannot be end-to-end triggered without a crafted fixture. The nil-copy-hint MECHANISM is proven end-to-end by the reflection cases (same `xeAutomationMutationNotAllowed` path). No product change required — the CLI hint (xeAutomationCommandsRecords.pas ~1038-1041) mirrors the native gate, oracle-source-confirmed.
- B-rebase PASS (re-witnesses the 2B reflection FormID remap on the CURRENT load order, superseding final.md's "mechanism-inferred" note): SFBGS050 WEAP BreechBlock 2D000976 reflection Ref file-local bytes `58 90 0A 02` rebased ON DISK to `58 90 0A 01` (SFBGS050 source-local master index 02 -> target-local 01; object index 0x0A9058 invariant). Witness `runs/redpill-20260714-161549/b-rebase/on-disk-byte-witness.txt`.
- B-pos six-signature coverage PASS: REFL / RDIF / PCCC / PTCL / PSDF / XNSE all copy `ok=true` under RedPill (decode-complete reflection is safe + remapped). `runs/.../b-pos-sixsig/`.
- B-neg-1 PASS: on a NON-RedPill daemon the same safe PNDT that RedPill copies is BLOCKED -> `mutation_not_allowed`, `Source contains Reflection and can not be copied`. Clean A/B control isolating the `wbStarfieldReverseEngineeringIncomplete` gate arm. `runs/nonredpill-20260714-165500/b-neg-1/`.
- B-neg-3 (malformed-CLAS block) NOT-COVERED / PENDING-USER-REPORT: byte-preserving `CLAS`->`CLAX` overlay fixture crafted, but the live block needs a daemon started AFTER MO2-enabling the overlay; running MO2 36472 (must-not-kill) rewrites the profile from memory, so enabling requires the user's MO2 GUI (same user-enabled-overlay dependency as 2A). The corpus is 100% walker-decode-complete, so there is no natural undecodable carrier to exercise in-session.

LOAD-BEARING clarification (source-confirmed, oracle-confirmed): `ContainsUnsafeReflection` (wbImplementation.pas:7086-7096) is DECODE-AWARE (rdsComplete=safe, slots remapped natively on copy; rdsUnsupported/rdsMalformed=unsafe, fail-closed), NOT the earlier 2C-3 "any CLAS Ref field => unsafe". The copy gate (wbImplementation.pas:16941) blocks iff `ContainsReflection AND (non-RedPill OR ContainsUnsafeReflection)`. Consequently the 17E-e matrix item "Ref-bearing record blocked with the new reason" is SUPERSEDED by 2B: under RedPill a Ref-bearing but decodable reflection COPIES + REMAPS (proven by B-rebase + B-pos), and is not blocked. The only RedPill reflection block is a decode-failure (malformed/unsupported grammar).

Later phases must do differently: (a) do NOT re-list the 17E-e "Ref-bearing blocked" case — it does not exist under current code; the real RedPill block is decode-failure only. (b) A3 unmapped-block and B-neg-3 malformed-block are PENDING-USER-REPORT (fixture-scarce / overlay-restart-dependent); reopen only with a real fixture (A3 needs a non-RedPill daemon + a record with a populated unmapped-FormID field; B-neg-3 needs a user-enabled malformed overlay + a daemon launched after enable). (c) reflection remap is now on-disk-witnessed on the current load order. (d) lane C (kind=9 reflection FormID -> target-FormType editor-UI dropdown filtering) remains a user-run GUI last mile; the wire layer already knows "this slot is a FormID" (kind=9), the missing piece is the target FormType for dropdown filtering, which is deferred UI-validation scope. (e) lane D upstream PR is permanently dropped.

## Phase 17 kind=9 reflection FormID slot surfacing — Stage A + B (2026-07-14)
Delivered: reflection "kind=9" embedded FormID slots (the form-references inside a decoded OBJT reflection payload, previously shown only as raw hex) are now surfaced in the xEdit GUI. All changes are in Core/wbInterface.pas on `TwbReflectionPayloadDef` (the value-def backing the OBJT/inline reflection payload), plus a small extensible whitelist. NO element-core change. Architecture chosen with @oracle: value-def display/edit overrides (def-local, upstreamable, zero element-core blast radius), NOT the Stage C "per-slot wbFormID child element + native filtered dropdown" (that needs a runtime-dynamic-member element-core rewrite and is deliberately deferred).
- Stage A (read-only decoded view): override `ToString`/`ToSummary` to reuse the reflection walker `Decode` and render each slot as `ClassName.FieldName = EditorID [SIG:FormID]` (resolved via a scratch wbFormID over the 4 slot bytes), with an optional target-FormType `-> <SIG>` annotation from the whitelist; falls back to raw hex when the payload does not fully decode (rdsUnsupported/rdsMalformed) — never hides an undecodable stream.
- Stage B (in-place text editing): override `GetIsEditable`/`GetEditType`/`ToEditValue`/`FromEditValue`. Editable only when the payload decodes complete; edit value is one `FieldName=<formid>` line per slot; `FromEditValue` re-decodes the LIVE leaf, validates FieldName+order, parses each new value through the wbFormID FORMATER (string -> file-local FormID, no storage mutation), and writes only the changed 4-byte slots in place (two-phase, atomic, fail-closed on any bad line).
- Target-FormType source: static `{className,fieldName} -> signature` whitelist `wbReflectionTargetSignatures` (18 high-confidence entries inferred from field names + CommonLibSF: pLightForm->LIGH, pImageSpace*->IMGS, pSpell*->SPEL, pImpactDataSet->IPDS, pSunPresetOverride/BGSSunPresetForm->SUNP, pDisplayNameKeyword->KYWD, pLensFlare->LENS, pExplosionForm->EXPL, pTrackedActorValue/pSource->AVIF, pParent(WeatherSettingsForm)->WTHR, ...). Unmapped fields DEGRADE to an unfiltered FormID (the resolved [SIG:...] still shows the real target); never a hard gate. The v7 live-process pointer-type-node RE for target FormType stays permanently deferred (hard, unnecessary).
- THREE regressions found and fixed during semantic verification — all were the new display/edit overrides silently interfering with the copy/serialize path (root-caused by @oracle each time):
  1. `GetIsEditable` fail-closed on the still-`SetToDefault(0)` TARGET leaf during `records.copy_into` -> flows through `CanAssignInternal` + the OBJT struct per-member `CanAssign` aggregation -> the whole OBJT member copy is SKIPPED (OBJT dropped). Fix: `if wbCopyIsRunning > 0 then Exit` (defer to inherited byte-editable during copy), mirroring the Step 2D conditional-formater escape.
  2. `TwbReflectionPayloadDef.Assign` called `inherited Assign` = `TwbDef.Assign` = a `SetEditValue(EditValue)` round-trip. Stage B overrode ToEditValue/FromEditValue to the semantic slot text, so during copy the round-trip emitted slot text and the target FromEditValue (Decode on the empty target) wrote ZERO bytes -> payload dropped. Fix: replace with `aTarget.NativeValue := aSource.NativeValue` (byte-faithful, resizing, bypasses the edit surface); the Step 2B slot rebase runs unchanged after.
  3. `FromEditValue` reused `wbFormID.FromEditValue(base,base+4,aElement,value)` -> `TwbIntegerDef.FromInt` -> `aElement.RequestStorageChange` RESIZED the whole variable-size payload leaf to 4 bytes on edit (payload 20->4, file 2371->2355). Fix: parse+translate through the wbFormID FORMATER (`lFormID.Formater[aElement].FromEditValue`, no storage change) and write `PCardinal(base+offset)^` directly.
- Semantic E2E (RedPill daemon, live SFBGS050 WEAP BreechBlock 2D000976, EffectSequenceMap.pParentForm slot): Stage A decoded view PASS (`= WeapFX_Breechblock_AmmoCount_Seq01 [EFSQ:2D0A9058]`, not hex). Copy byte-exact PASS (on-disk OBJT payload intact, file 2371, reflection Ref rebased file-local 02->01, Step 2B preserved). Stage B edit PASS (set slot to 0000015E bare-hex -> readback `DefaultWeather [WTHR:0000015E]` decoded, file size 2371 UNCHANGED, payload 20 bytes intact, slot bytes `5E 01 00 00` file-local correct). Stage B malformed (`ZZZZ`) fail-closed PASS (zero bytes written, readback uncorrupted). Evidence under `.opencode/artifacts/phase17-kind9-reflection-ui/runs/redpill-fix3-*/verify`.
