...
The current local truth on this branch is now the accepted Phase 11 navigate-to-record follow-up on top of the scoped xEdit-side `1.0` freeze:
...
- Phase 6B-E final local-only closeout is now complete:
  ...
  - intentionally deferred GUI-coupled work remains outside 6B-E: arbitrary dialog/button automation beyond the coarse `session.get_gui_snapshot` blocker probe, full Pascal script execution, medium-plugin support, arbitrary record-signature construction/cleaning semantics, broad query DSLs, and any workflow macro layer that would hide the atomic command/job boundary
...
- Phase 5 patch-building primitives are now implemented locally and runtime-verified through the MO2 Fallout 4 harness:
  ...
  - `records.create` for the bounded `KYWD` and `MISC` allow-list
  ...
  - `system.capabilities` now reports contract `0.5`, Phase 5 command names, create-file extensions/flags, and `records.create` signatures
...
- We now know `records.create` should stay intentionally narrow for now: `KYWD` and `MISC` cover useful patch-building scaffolding, while higher-risk signatures such as `NPC_` remain out of scope until a later design covers required defaults, templates, and side effects.
...
- We now know some GUI-coupled work should stay deferred rather than smuggled into Phase 6 job handlers: arbitrary modal/button control, Pascal script execution, broad workflow macros, medium-plugin policy, arbitrary record-signature semantics, and unbounded query behavior need later explicit design instead of incidental 6B-E expansion.
...
- Add record-level copy/create/delete primitives where they can be bounded safely.
  - Local implementation now includes `records.create` for `KYWD` and `MISC`, `records.copy_into`, `records.delete`, and `records.mark_deleted`.
...
- Contract `0.5` is the Phase 5 surface marker; clients can discover supported create-file extensions/flags and the `records.create` signature allow-list through `system.capabilities`.
- Intentionally unsupported Phase 5 variants remain out of scope: medium plugins, arbitrary record signatures, child-path delete/mark-delete, direct protected-file mutation, workflow macros, and implicit save-on-mutation behavior.
